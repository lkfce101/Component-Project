...
My choice of representation will be to use a sequence as it is the easiest structure for me to be able to implement the kernel methods that I wrote. An empty card deck is just an empty sequence and then the order of the card deck (read left to right) is the same as the sequence.
